...
public class CommonTransformer<S, T> implements Function<S, T> {
    ...
    public <O> CommonTransformer<S, T> fields(final Function<S, O> getter, final BiConsumer<T, O> setter) {
        objectFields.add((source, target) -> setter.accept(target, getter.apply(source)));
        return this;
    }

    public <O1, O2> CommonTransformer<S, T> fields(final Function<S, O1> getter, final BiConsumer<T, O2> setter, final Function<O1, O2> converter) {
    ...
    public CommonTransformer<S, T> constructor(final Supplier<T> constructor) {
        this.objectConstructor = (dontuse) -> constructor.get();
        return this;
    }

    public <O> CommonTransformer<S, T> constructor(final Function<O, T> construct, final Function<S, O> param1) {
        this.objectConstructor = (S source) -> construct.apply(param1.apply(source));
        return this;
    }

    public <O1, O2> CommonTransformer<S, T> constructor(final BiFunction<O1, O2, T> construct, final Function<S, O1> param1, final Function<S, O2> param2) {
        this.objectConstructor = (S source) -> construct.apply(param1.apply(source), param2.apply(source));
        return this;
    }

    public static <S, T> CommonTransformer<S, T> mapping(final Class<S> source, final Class<T> target) {
